...
//	static float PWM_M4=0;
	...
	Encoder_UpdataValue(&Encoder_M1, 1);//更新M1编码器数值
//	Encoder_UpdataValue(&Encoder_M2, -1); //更新M2编码器数值
	Encoder_UpdataValue(&Encoder_M3, -1);//更新M3编码器数值
//	Encoder_UpdataValue(&Encoder_M4, -1); //更新M4编码器数值
...
//	PWM_M4 = Constrain(PWM_M4, -MotorPWM_Max, MotorPWM_Max);
	...
	if(PWM_M3 > 0.0f) 
	{
		Motor2_PWM = PWM_M3;//控制电机2 PWM输出  MotorPWM_Max-
...
//	Motor1_PWM = PWM_VAL;
	...
	if(PWM_M3 > 0.0f) 
	{
		Motor3_PWM = PWM_M3;//控制电机3 PWM输出MotorPWM_Max-
	...
	if(PWM_M4 > 0.0f) 
	{
		Motor4_PWM = PWM_M4;//控制电机4 PWM输出MotorPWM_Max-
